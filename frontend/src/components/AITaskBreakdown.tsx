...
export const AITaskBreakdown: React.FC<AITaskBreakdownProps> = ({
  ...
  const createSubtasksMutation = useMutation({
    ...
    onSuccess: (createdTasks) => {
      setShowReview(false);
      setSuccessMessage(`✅ Created ${createdTasks.length} subtasks!`);
      onSubtasksGenerated(createdTasks.map(t => t.data));
    },
    onError: (error: any) => {
      setSuccessMessage('❌ Failed to create subtasks');
    },
  });
  ...
  return (
    <>
      ...
      >
        <Alert severity={successMessage.startsWith('✅') ? 'success' : 'error'}>
          {successMessage}
        </Alert>
      </Snackbar>
    </>
