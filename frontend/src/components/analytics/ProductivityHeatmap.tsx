...
const ProductivityHeatmap: React.FC = () => {
  ...
  const { data: rangeData } = useAnalyticsRange(84); // Last 12 weeks
    ...
    <Paper sx={{ p: 3, mb: 3 }}>
      ...
      <Box sx={{ overflowX: "auto", pb: 2 }}>
        <Box display="flex" gap={`${cellGap}px`}>
          {/* Day labels */}
          <Box display="flex" flexDirection="column" justifyContent="space-around" mr={1}>
            {dayLabels.map((label, idx) => (
              <Typography
                ...
                sx={{
                  ...
                  fontSize: "10px",
                  visibility: idx % 2 === 0 ? "visible" : "hidden", // Show every other label
                }}
              >
                {label}
              </Typography>
          ...
          {weeks.map((week, weekIdx) => (
            <Box key={weekIdx} display="flex" flexDirection="column" gap={`${cellGap}px`}>
                    ...
                    title={
                      day ? (
                        <Box>
                          ...
                          <Typography variant="caption" display="block">
                            Focus: {Math.floor(focusMinutes / 60)}h {focusMinutes % 60}m
                          </Typography>
                        </Box>
                      ) : (
                        "No data"
                  ...
                  >
                    <Box
                      sx={{
                        width: `${cellSize}px`,
                        height: `${cellSize}px`,
                        backgroundColor: day ? getColor(score) : theme.palette.action.disabledBackground,
                        borderRadius: "2px",
