...
public class AnalyticsCalculationService {
    ...
    public ProductivityInsightsDTO calculateProductivityInsights(Long userId) {
        ...
        for (LocalDate date = startDate; !date.isAfter(endDate); date = date.plusDays(1)) {
            ...
            if (analytics.getPomodorosCompleted() > 0
                    || analytics.getTasksCompleted() > 0) {
                daysAnalyzed++;

                // Track by day of week
                DayOfWeek dayOfWeek = date.getDayOfWeek();
                dayScores.merge(
                        dayOfWeek, analytics.getProductivityScore(), (a, b) -> a + b);

                // Track time of day
                timeOfDayMinutes.merge(
                        "morning", analytics.getMorningFocusMinutes(), Integer::sum);
                timeOfDayMinutes.merge(
        ...
        int averageSessionLength =
                sessionLengths.isEmpty()
                        ? 0
                        : (int) sessionLengths.stream().mapToInt(Integer::intValue).average().orElse(0);
    ...
    private double calculateTrend(List<Double> values) {
        ...
        double firstAvg =
                firstHalf.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
        double secondAvg =
