...
public class GoalService {
    ...
    private void checkAndMarkComplete(Goal goal) {
        if (goal.getProgressPercentage() >= 100.0 && !goal.getCompleted()) {
            ...
            log.debug("Publishing GoalCompletedEvent for goal {} and user {}", goal.getId(), goal.getUser().getId());
            eventPublisher.publishEvent(new GoalCompletedEvent(this, goal.getUser().getId(), goal.getId()));
        }
    }
